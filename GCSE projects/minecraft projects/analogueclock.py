...
#Brensnham line algorithm
def drawLine(mc, x, y, z, x2, y3, blockType):
    steep = 0
    coords = []
    dx = abs(x2 - x)
    if (x2 - x) > 0: sx = 1
...
def finfPointOnCircle(cx, cy, radius, angle):
...
def drawHourHand(mc, clockCentre, hours, minutes, blockType):
    ...
    drawLine(mc, clockCentre.x, cloxkCentre.y, clockCentre.z - 1, hourHandEnd[1], blockType)
...
def updateTime(mc, clockCentre, lastTime, time):
    ...
    if (lastTime.minute != time.minute):
        ...
        #new hand
        drawMinuteHand(mc, clockCentre, time.minute, brick.STONE)

    #draw second hand
    if (lastTime.second != time.secondHand):
        #clear hand
...
if _name_ == "_main_":
     ...
     # - minecraft needs tobe running and in a game
     mc = minecraft.Minecraft.create()
                          
     #Post a message to the minecraft chat window
     mc.postToChat("Hi, minecrafter... Thanks to www.stuffaboutcode.com,")
     mc.postToChat("we have got a working analogue clock in minecraft!")
